...
interface MessagingState extends ConversationState, MessageState, FriendState, PresenceState, NotificationState, UIState {
  ...
  getFriendshipStatusWithUser: (userId: string) => 'friends' | 'pending_sent' | 'pending_received' | 'not_friends';
...
export const useMessagingStore = create<MessagingState>()(
  subscribeWithSelector((set, get) => ({
    ...
    // Conversation actions
    setConversations: (conversations) => set({ conversations: sortConversationsByLastMessage(conversations) }),
    
    addConversation: (conversation) => set((state) => ({
      conversations: sortConversationsByLastMessage([conversation, ...state.conversations])
    })),
    ...
    addMessage: (message) => set((state) => {
      const conversationMessages = state.messagesByConversation[message.conversation_id] || [];
      const updatedMessages = sortMessagesByTimestamp([...conversationMessages, message]);
      ...
      const updatedConversations = state.conversations.map(conv =>
        conv.id === message.conversation_id
          ? {
              ...
              unread_count: message.sender.id !== state.currentUserId ? conv.unread_count + 1 : conv.unread_count
            }
          : conv
      );
    ...
    removeMessage: (messageId) => set((state) => {
      ...
      Object.keys(newMessagesByConversation).forEach(conversationId => {
        newMessagesByConversation[conversationId] = newMessagesByConversation[conversationId].filter(msg =>
          msg.id !== messageId
        );
      });
    ...
    setMessageLoading: (conversationId, loading) => set((state) => ({
      loadingMessages: { ...state.loadingMessages, [conversationId]: loading }
    })),
    
    setMessageLoadingMore: (conversationId, loading) => set((state) => ({
      loadingMoreMessages: { ...state.loadingMoreMessages, [conversationId]: loading }
    })),
    
    setMessageError: (conversationId, error) => set((state) => ({
    ...
    searchMessagesInConversation: (conversationId, query) => set((state) => {
    ...
    addFriendship: (friendship) => set((state) => ({
      friendships: [...state.friendships, friendship]
    })),
    
    removeFriendship: (friendshipId) => set((state) => ({
    ...
    setFriendRequests: (requests) => set((state) => {
      ...
      const friendRequests = requests.filter(req => req.recipient.id === currentUserId);
      const sentFriendRequests = requests.filter(req => req.requester.id === currentUserId);
    ...
    removeFriendRequest: (requestId) => set((state) => ({
      ...
      sentFriendRequests: state.sentFriendRequests.filter(req => req.id !== requestId)
    })),
    ...
    setMultipleUserPresence: (presenceMap) => set((state) => {
    ...
    updateLastPresenceUpdate: () => set({ lastPresenceUpdate: new Date().toISOString() }),
    ...
    addNotification: (notification) => set((state) => ({
      ...
      unreadNotificationCount: !notification.read ? state.unreadNotificationCount + 1 : state.unreadNotificationCount
    })),
    ...
    setNotificationPermission: (permissionGranted) => set({ permissionGranted }),
    ...
    getFilteredConversations: () => {
      ...
      if (state.searchQuery && state.currentUserId) {
        filtered = searchConversations(filtered, state.searchQuery, state.currentUserId);
      }
    ...
    getFriendshipStatusWithUser: (userId) => {
      ...
      return getFriendshipStatus(state.currentUserId, userId, state.friendships, [...state.friendRequests, ...state.sentFriendRequests]);
    },
    
    canMessageUser: (userId) => {
      const state = get();
      if (!state.currentUserId) return { canSend: false, reason: 'Not authenticated' };
      return canSendMessage(state.currentUserId, userId, state.friendships, []);
    },
    
    canCallUser: (userId) => {
      const state = get();
      if (!state.currentUserId) return { canCall: false, reason: 'Not authenticated' };
      return canInitiateCall(state.currentUserId, userId, state.friendships, []);
    },
    ...
    getFriendsList: () => {
      ...
      return state.friendships
        .filter(friendship => 
          friendship.user1.id === state.currentUserId || friendship.user2.id === state.currentUserId
        )
        .map(friendship => 
          friendship.user1.id === state.currentUserId ? friendship.user2 : friendship.user1
        );
    },
      ...
      switch (message.type) {
        ...
        case 'typing_start':
          if (message.payload?.conversation_id && message.payload?.user_id) {
            get().addTypingUser(message.payload.conversation_id, message.payload.user_id);
          }
          break;
          
        case 'typing_stop':
          if (message.payload?.conversation_id && message.payload?.user_id) {
            get().removeTypingUser(message.payload.conversation_id, message.payload.user_id);
          }
          break;
        ...
        case 'presence_update':
          if (message.payload?.user_id && message.payload?.status) {
            get().setUserPresence(message.payload.user_id, {
              ...
              last_seen_at: message.payload.last_seen_at || new Date().toISOString(),
