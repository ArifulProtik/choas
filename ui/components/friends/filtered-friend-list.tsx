...
import { MessageCircle, MoreHorizontal, Phone, UserMinus } from "lucide-react";
import { useState } from "react";

interface FilteredFriendListProps {
  friends: User[];
  filterType: "all" | "online" | "offline";
...
export function FilteredFriendList({
  ...
  onStartConversation,
  onStartCall,
}: FilteredFriendListProps) {
  const [removingFriend, setRemovingFriend] = useState<User | null>(null);
  const [isRemoving, setIsRemoving] = useState(false);

  const { getUserPresence, removeFriendship, friendships, currentUserId } =
    useMessagingStore();
    ...
    switch (filterType) {
      ...
      case "offline":
        return {
          ...
          description: "All your friends are currently online",
        };
      default:
        return {
          icon: <MessageCircle className="w-8 h-8" />,
