...
func (mq *MemberQuery) loadGuild(ctx context.Context, query *GuildQuery, nodes []*Member, init func(*Member), assign func(*Member, *Guild)) error {
	...
	for i := range nodes {
		if nodes[i].guild_guild_members == nil {
			continue
		}
		fk := *nodes[i].guild_guild_members
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "guild_guild_members" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MemberQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Member, init func(*Member), assign func(*Member, *User)) error {
	...
	nodeids := make(map[string][]*Member)
	for i := range nodes {
		if nodes[i].user_member_of == nil {
			continue
		}
		fk := *nodes[i].user_member_of
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_member_of" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
