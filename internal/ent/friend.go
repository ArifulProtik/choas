...
type Friend struct {
	...
	// AddresseeID holds the value of the "addressee_id" field.
...
type FriendEdges struct {
	...
	// Addressee holds the value of the addressee edge.
...
// AddresseeOrErr returns the Addressee value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e FriendEdges) AddresseeOrErr() (*User, error) {
	if e.Addressee != nil {
		return e.Addressee, nil
	} else if e.loadedTypes[1] {
		return nil, &NotFoundError{label: user.Label}
	}
	return nil, &NotLoadedError{edge: "addressee"}
}
...
func (*Friend) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case friend.FieldID, friend.FieldRequesterID, friend.FieldAddresseeID, friend.FieldStatus:
	...
	for i := range columns {
		...
		case friend.FieldRequesterID:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				f.RequesterID = value.String
			}
		case friend.FieldAddresseeID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field addressee_id", values[i])
			} else if value.Valid {
				f.AddresseeID = value.String
			}
...
// QueryAddressee queries the "addressee" edge of the Friend entity.
func (f *Friend) QueryAddressee() *UserQuery {
	return NewFriendClient(f.config).QueryAddressee(f)
}
...
func (f *Friend) String() string {
	...
	builder.WriteString(f.RequesterID)
	builder.WriteString(", ")
	builder.WriteString("addressee_id=")
	builder.WriteString(f.AddresseeID)
	builder.WriteString(", ")
