...
type NotificationQuery struct {
	...
	withUser                *UserQuery
	withRelatedUser         *UserQuery
	withRelatedConversation *ConversationQuery
...
// QueryRelatedUser chains the current query on the "related_user" edge.
func (nq *NotificationQuery) QueryRelatedUser() *UserQuery {
	query := (&UserClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryRelatedConversation chains the current query on the "related_conversation" edge.
func (nq *NotificationQuery) QueryRelatedConversation() *ConversationQuery {
	query := (&ConversationClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withRelatedUser:         nq.withRelatedUser.Clone(),
		withRelatedConversation: nq.withRelatedConversation.Clone(),
...
// WithRelatedUser tells the query-builder to eager-load the nodes that are connected to
// the "related_user" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithRelatedUser(opts ...func(*UserQuery)) *NotificationQuery {
	query := (&UserClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withRelatedUser = query
	return nq
}

// WithRelatedConversation tells the query-builder to eager-load the nodes that are connected to
// the "related_conversation" edge. The optional arguments are used to configure the query builder of the edge.
func (nq *NotificationQuery) WithRelatedConversation(opts ...func(*ConversationQuery)) *NotificationQuery {
	query := (&ConversationClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withRelatedConversation = query
	return nq
}
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	var (
		...
		loadedTypes = [3]bool{
			nq.withUser != nil,
			nq.withRelatedUser != nil,
			nq.withRelatedConversation != nil,
		}
	)
	...
	if query := nq.withRelatedUser; query != nil {
		if err := nq.loadRelatedUser(ctx, query, nodes, nil,
			func(n *Notification, e *User) { n.Edges.RelatedUser = e }); err != nil {
			return nil, err
		}
	}
	if query := nq.withRelatedConversation; query != nil {
		if err := nq.loadRelatedConversation(ctx, query, nodes, nil,
			func(n *Notification, e *Conversation) { n.Edges.RelatedConversation = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NotificationQuery) loadRelatedUser(ctx context.Context, query *UserQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Notification)
	for i := range nodes {
		fk := nodes[i].RelatedUserID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "related_user_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NotificationQuery) loadRelatedConversation(ctx context.Context, query *ConversationQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Conversation)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Notification)
	for i := range nodes {
		fk := nodes[i].RelatedConversationID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "related_conversation_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NotificationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := nq.ctx.Fields; len(fields) > 0 {
		...
		if nq.withRelatedUser != nil {
			_spec.Node.AddColumnOnce(notification.FieldRelatedUserID)
		}
		if nq.withRelatedConversation != nil {
			_spec.Node.AddColumnOnce(notification.FieldRelatedConversationID)
		}
	}
