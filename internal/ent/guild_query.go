...
func (gq *GuildQuery) QueryInvitations() *InvitationQuery {
	query := (&InvitationClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryGuildMembers chains the current query on the "guild_members" edge.
func (gq *GuildQuery) QueryGuildMembers() *MemberQuery {
...
func (gq *GuildQuery) Clone() *GuildQuery {
	...
	return &GuildQuery{
		...
		withInvitations:  gq.withInvitations.Clone(),
		withGuildMembers: gq.withGuildMembers.Clone(),
...
func (gq *GuildQuery) WithInvitations(opts ...func(*InvitationQuery)) *GuildQuery {
	query := (&InvitationClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withInvitations = query
	return gq
}

// WithGuildMembers tells the query-builder to eager-load the nodes that are connected to
// the "guild_members" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GuildQuery) WithGuildMembers(opts ...func(*MemberQuery)) *GuildQuery {
	query := (&MemberClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGuildMembers = query
	return gq
}
...
func (gq *GuildQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Guild, error) {
	var (
		...
		loadedTypes = [3]bool{
			gq.withOwner != nil,
			gq.withInvitations != nil,
			gq.withGuildMembers != nil,
		}
	)
	if gq.withOwner != nil {
		withFKs = true
	...
	if query := gq.withInvitations; query != nil {
		if err := gq.loadInvitations(ctx, query, nodes,
			func(n *Guild) { n.Edges.Invitations = []*Invitation{} },
			func(n *Guild, e *Invitation) { n.Edges.Invitations = append(n.Edges.Invitations, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withGuildMembers; query != nil {
		if err := gq.loadGuildMembers(ctx, query, nodes,
			func(n *Guild) { n.Edges.GuildMembers = []*Member{} },
			func(n *Guild, e *Member) { n.Edges.GuildMembers = append(n.Edges.GuildMembers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GuildQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Guild, init func(*Guild), assign func(*Guild, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_owned_guilds == nil {
			continue
		}
		fk := *nodes[i].user_owned_guilds
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_owned_guilds" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (gq *GuildQuery) loadInvitations(ctx context.Context, query *InvitationQuery, nodes []*Guild, init func(*Guild), assign func(*Guild, *Invitation)) error {
	...
	query.Where(predicate.Invitation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(guild.InvitationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.invitation_guild
		if fk == nil {
			return fmt.Errorf(`foreign-key "invitation_guild" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "invitation_guild" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (gq *GuildQuery) loadGuildMembers(ctx context.Context, query *MemberQuery, nodes []*Guild, init func(*Guild), assign func(*Guild, *Member)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Guild)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Member(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(guild.GuildMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.guild_guild_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "guild_guild_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "guild_guild_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
