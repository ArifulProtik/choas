...
// QueryAddressee chains the current query on the "addressee" edge.
func (fq *FriendQuery) QueryAddressee() *UserQuery {
	query := (&UserClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (fq *FriendQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (fq *FriendQuery) FirstIDX(ctx context.Context) string {
...
func (fq *FriendQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (fq *FriendQuery) OnlyIDX(ctx context.Context) string {
...
func (fq *FriendQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (fq *FriendQuery) IDsX(ctx context.Context) []string {
	ids, err := fq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (fq *FriendQuery) Clone() *FriendQuery {
	...
	return &FriendQuery{
		...
		withAddressee: fq.withAddressee.Clone(),
...
// WithAddressee tells the query-builder to eager-load the nodes that are connected to
// the "addressee" edge. The optional arguments are used to configure the query builder of the edge.
func (fq *FriendQuery) WithAddressee(opts ...func(*UserQuery)) *FriendQuery {
	query := (&UserClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withAddressee = query
	return fq
}
...
func (fq *FriendQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Friend, error) {
	...
	if query := fq.withAddressee; query != nil {
		if err := fq.loadAddressee(ctx, query, nodes, nil,
			func(n *Friend, e *User) { n.Edges.Addressee = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FriendQuery) loadRequester(ctx context.Context, query *UserQuery, nodes []*Friend, init func(*Friend), assign func(*Friend, *User)) error {
	...
	nodeids := make(map[string][]*Friend)
	for i := range nodes {
...
func (fq *FriendQuery) loadAddressee(ctx context.Context, query *UserQuery, nodes []*Friend, init func(*Friend), assign func(*Friend, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Friend)
	for i := range nodes {
		fk := nodes[i].AddresseeID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "addressee_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (fq *FriendQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(friend.Table, friend.Columns, sqlgraph.NewFieldSpec(friend.FieldID, field.TypeString))
	_spec.From = fq.sql
	...
	if fields := fq.ctx.Fields; len(fields) > 0 {
		...
		if fq.withAddressee != nil {
			_spec.Node.AddColumnOnce(friend.FieldAddresseeID)
		}
	}
