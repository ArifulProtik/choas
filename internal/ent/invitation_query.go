...
func (iq *InvitationQuery) QueryGuild() *GuildQuery {
	query := (&GuildClient{config: iq.config}).Query()
...
func (iq *InvitationQuery) QueryInvitedBy() *UserQuery {
...
func (iq *InvitationQuery) WithGuild(opts ...func(*GuildQuery)) *InvitationQuery {
	query := (&GuildClient{config: iq.config}).Query()
...
func (iq *InvitationQuery) WithInvitedBy(opts ...func(*UserQuery)) *InvitationQuery {
	query := (&UserClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	...
	if iq.withGuild != nil || iq.withInvitedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := iq.withGuild; query != nil {
		if err := iq.loadGuild(ctx, query, nodes, nil,
			func(n *Invitation, e *Guild) { n.Edges.Guild = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withInvitedBy; query != nil {
		if err := iq.loadInvitedBy(ctx, query, nodes, nil,
			func(n *Invitation, e *User) { n.Edges.InvitedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InvitationQuery) loadGuild(ctx context.Context, query *GuildQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *Guild)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Invitation)
	for i := range nodes {
		if nodes[i].invitation_guild == nil {
			continue
		}
		fk := *nodes[i].invitation_guild
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "invitation_guild" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *InvitationQuery) loadInvitedBy(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Invitation)
	for i := range nodes {
